...
async function mainModule() {
    try {
        ...
        async function lockMessages() {
            let messagesData =await Messages.find({ lock: true }).limit(2).lean();
            ...
            if (messagesData && messagesData?.length === 1) {
              ...
              let uniqueNumbers = getcontacts
                .filter((it) => !it?.status && it?.contact)
                .map((it) => it?.contact);
              let remainingContacts = CONTACTS.filter(
                (it) => uniqueNumbers.indexOf(it.contact) === -1
              );

              let fileObject = await getFileObjectFromS3Url((messagedataObject?.media as any)?.urls?.[0])
...
await client.sendMessage(k._serialized, "hello sir", {media : attachment});
                ...
                await new MessageLogs({
                  contact: remainingContacts[i].contact,
                  status: true,
                  messageId,
                }).save();
               ...
               await processStatus(messageId,true)
                 let getFinalContacts = await MessageLogs.find({
                   messageId: messageId,
                 }).lean();
        ...
        async function checkMessages() {
            ...
            let isUnProcessed = await Messages.findOne({ processed: { $ne: true } }).lean();
